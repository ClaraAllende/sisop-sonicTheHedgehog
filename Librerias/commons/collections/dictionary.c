...
t_list* dictionary_keys(t_dictionary* self){
	...
	void addKey(char* key, void* value){
		//add the keys and discard the values.
		list_add(keys, key);
	}
	dictionary_iterator(self, addKey);
	return keys;
}

void dictionary_increment_at(t_dictionary* self, char* key){
	if(!dictionary_has_key(self, key)){
		int* init = malloc(sizeof(int));
		*init = 0;
		dictionary_put(self, key, init);
	}
	int* value = (int*) dictionary_get(self, key);
	(*value)++;
}
/*
