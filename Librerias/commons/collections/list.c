...
bool list_all(t_list* original, bool(*condition)(void*)){
	t_list* filtered = list_filter(original, condition);
	bool ret = list_size(original) == list_size(filtered);
	free(filtered);
	return ret;
}

bool list_any(t_list* original, bool(*condition)(void*)){
	t_list* filtered = list_filter(original, condition);
	bool ret = list_size(filtered)> 0;
	free(filtered);
	return ret;
}
